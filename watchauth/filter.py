	...
	for userID in userIDs:
		for param in params:
			...
			for is_gesture in activities:
				f_outfilename = '../data/filtered/'+userID +  '-gestures.csv' if is_gesture else '../data/filtered/'+userID + '-nongestures.csv'
				if os.path.exists(f_outfilename) and not is_gesture and not is_overwritenongestures:
					break
				
				file = '../data/original/'+userID + '-gestures.csv' if is_gesture else '../data/original/'+userID + '-nongestures.csv'
